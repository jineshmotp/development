...
const PropertyBroucher: React.FC<OwnerDetailsProps> = ({
  control,
  controlConstraints,
  errors,
  setDetails,
  ...
  useEffect(() => {
    ...
    setDetails({ ...details, property_broucher: imgArray });
  }, [imgArray]);

  //######################### update image

  useEffect(() => {
    console.log('property browsher-------', details?.property_broucher);

    if (imgArray.length > 0) {
      const transformedArray = details?.property_broucher.map(url => ({
        url: url,
        id: Math.floor(Math.random() * 1400),
      }));
    ...
    // setUploadImgArray(details?.property_broucher);
  }, [details?.property_broucher]);

  return (
    <>
      <RNView>
        <RNView style={uploadstyles.container}>
          <SectionHoc title="Property Brochure" mandatory={mandatory}>
...
export default PropertyBroucher;
