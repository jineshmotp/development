...
import HighlightExpand from '@/components/property/HighlightExpand';
...
const BusinessHighlights: React.FC<PropertyAreaProps> = ({ details }) => {
  ...
  useEffect(() => {
    setTimeout(() => {
      const initialPayload = `${details?.location?.coordinates[0]},${details?.location?.coordinates[1]}&type=${highlightD[0].label}`;
  ...
  const activateKeyForHighlight = (item: any) => {
    sethighlightDetails([]);
    const newPayload = `${details?.location?.coordinates[0]},${details?.location?.coordinates[1]}&type=${item.label}`;
      ...
      <>
        ...
        <RNView style={styles.mapViewView}>
          <MapView
            ...
            loadingEnabled={true}
            initialRegion={{
              latitude: details?.location?.coordinates[0] ?? 17.44707484820716,
              longitude: details?.location?.coordinates[1] ?? 78.35418918766196,
              latitudeDelta: 0.0922,
              longitudeDelta: 0.0421,
            }}
            zoomControlEnabled={true}
            style={styles.mapStyle}>
            {(details?.location?.coordinates[0] !== null || details?.location?.coordinates[0] !== undefined) && (
              <Marker
                coordinate={{
                  latitude: details?.location?.coordinates[0] ?? 17.44707484820716,
                  longitude: details?.location?.coordinates[1] ?? 78.35418918766196,
