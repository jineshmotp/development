...
import { ProperyListing_PayloadGenerator } from '@/constants/function/builder.payloadFunction';
import CommonButton from '@/custom/CommonButton';
import { Container } from '@/custom/Container';
import { RNText } from '@/custom/RNText';
import RNView from '@/custom/RNView';
import { useAppDispatch, useAppSelector } from '@/hooks/reduxHooks';
import { selectBusinessBasicData } from '@/redux/builder/builderReducer';
...
import { useGetAllSubscriptionOptionsQuery } from '@/redux/listing/listingService';
...
const BuilderPostPropertyStepFive = () => {
  const selectedUserData = useAppSelector(getUserData);

  const dispatch = useAppDispatch();
  const route = useRoute<RouteProp<RootStackParamList, 'BUILDER_POST_PROPERTY_FIVE'>>();
  const currentStep = 5;
  const toast = useToast();
  const [uploadListingMutation] = useBuilderListaddingMutation({});

  const { data, status } = useGetAllSubscriptionOptionsQuery({});
  ...
  const businessdata = useAppSelector(selectBusinessBasicData);

  // console.log(' businesss data: ', businessdata);
  ...
  const onSubmit = data1 => {
    ...
    if (retvalfinal === 1) {
      ...
      // console.log('newretpayload subscription ------->', newretpayload);
...
export default BuilderPostPropertyStepFive;
