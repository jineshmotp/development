...
const PostPropertyStepThreeEditing = () => {
  const route = useRoute<RouteProp<RootStackParamList, 'POST_PROPERTY_THREE_EDITING'>>();
  const currentStep = 3;
  ...
  const { data } = useGetPropertyDetailsQuery({
    property: route?.params?.post2?.BasicData?._id,
  ...
  const onSubmit = data => {
    //console.log(' page 3 Details : ', propertyDiscription);
...
export default PostPropertyStepThreeEditing;
