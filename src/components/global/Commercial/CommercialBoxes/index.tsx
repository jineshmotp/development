...
import CommercialRangeSlider from '../CommercialRangeSlider';
import { styles } from './styles';

export const CommercialRangeFilter = ({ filterDetails, setFilterDetails }: any) => {
  return (
    <RNView style={styles.filterView}>
      <CommercialRangeSlider filterDetails={filterDetails} setFilterDetails={setFilterDetails} />
    </RNView>
  );
};
...
export const BareShellTypeFilterData = ({ filterDetails, setFilterDetails }: any) => {
  const convertRequired = (data, propertyType) => {
    // console.log("propertyTpe", propertyType);
    const temp = data.map(item => {
      if (propertyType.includes(item?.label)) {
  ...
  const [multipleSelectable, setMultipleSelectable] = useState(() =>
    convertRequired(BareShellData, filterDetails?.bareshell)
  );
  ...
  const changeStatus = item => {
    ...
    setFilterDetails({
      ...filterDetails,
      bareshell: multipleSelected.filter((item: any) => item.active === true).map((item: any) => item.label),
    });
...
export const PropertyTypeFilter = ({ filterDetails, setFilterDetails }: any) => {
  ...
  const [multipleSelectable, setMultipleSelectable] = useState(() =>
    convertRequired(CommercialFilterData, filterDetails?.property_type)
  );
