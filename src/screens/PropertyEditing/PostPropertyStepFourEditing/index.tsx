...
const PostPropertyStepFourEditing = () => {
  const route = useRoute<RouteProp<RootStackParamList, 'POST_PROPERTY_FOUR_EDITING'>>();
  const currentStep = 4;
  ...
  const { data } = useGetPropertyDetailsQuery({
    property: route?.params?.post3?.BasicData?._id,
  ...
  const onSubmit = data => {
    ...
    if (retvalfive === 1) {
      ...
      navigation.navigate('POST_PROPERTY_FIVE_EDITING', {
        post4: {
          ...route?.params?.post3,
          ...propertyDiscription,
          BasicData: route?.params?.post3?.BasicData,
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      const ownership_val = data?.data?.property?.ownership;
...
export default PostPropertyStepFourEditing;
