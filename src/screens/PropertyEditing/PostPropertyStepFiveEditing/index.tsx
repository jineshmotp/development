...
import { ProperyListing_PayloadGenerator } from '@/constants/function/property.payloadEditing';
...
const PostPropertyStepFiveEditing = () => {
  const route = useRoute<RouteProp<RootStackParamList, 'POST_PROPERTY_FIVE_EDITING'>>();
  ...
  const [updatePropertyMutation, { status }] = useUpdatePropertyApiMutation({});

  const { data } = useGetPropertyDetailsQuery({
    property: route?.params?.post4?.BasicData?._id,
...
export default PostPropertyStepFiveEditing;
